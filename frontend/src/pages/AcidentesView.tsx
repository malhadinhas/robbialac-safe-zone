      ...
      <div className="min-h-[80vh] bg-[#f7faff] py-8 px-2 sm:px-6">
        <div className="container mx-auto max-w-4xl flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-md p-4 flex flex-col gap-4 border border-slate-100">
            <div className="mb-1">
              <h1 className="text-lg font-bold break-words leading-tight">{doc?.name || 'Documento de Acidente'}</h1>
              <p className="text-gray-600 text-sm">Detalhes do Acidente.</p>
            </div>
            {loading ? (
              <div className="flex items-center justify-center h-40 text-lg">A carregar...</div>
            ) : doc && doc.pdfUrl ? (
              <Document file={doc.pdfUrl} loading={<div className="text-gray-400">A carregar PDF...</div>} error={<div className="text-red-400">Erro ao carregar PDF</div>} className="w-full h-full">
