...
const checkActionBasedMedals = async (userId, activityCategory, activityDetails // Detalhes podem conter 'category' para vídeo/treino
    ...
    try {
        ...
        logger_1.default.debug(`Encontradas ${potentialMedals.length} medalhas potenciais para ${targetTriggerAction}.`);
        ...
        if (medalsToCheck.length === 0) {
            logger_1.default.info(`Usuário ${userId} já possui todas as ${potentialMedals.length} medalhas disponíveis para ${targetTriggerAction} ou não há medalhas novas.`);
            return [];
        }
...
const createMedal = async (req, res) => {
    try {
        ...
        if (existingMedal) {
            logger_1.default.warn(`Tentativa de criar medalha com ID duplicado: ${medalData.id}`);
        ...
        const result = await collection.insertOne(medalToInsert);
        logger_1.default.info(`Nova medalha criada com sucesso: ${medalData.name} (ID: ${medalData.id}), _id: ${result.insertedId}`);
...
const updateMedal = async (req, res) => {
    try {
        ...
        if (!existingMedal) {
            ...
            logger_1.default.info('Medalhas disponíveis no sistema:', { medals: allMedals.map(m => m.id) });
        ...
        delete updateData.id; // Remove a propriedade 'id' se existir em updateData.
        // Executa a operação `updateOne` no MongoDB.
        const result = await collection.updateOne({ id: medalId }, // Critério de filtro: encontra a medalha pelo seu ID string (slug).
        ...
        if (result.matchedCount === 0) {
            ...
            logger_1.default.warn(`Tentativa de atualizar medalha não encontrada (após verificação findOne): ${medalId}`);
            return res.status(404).json({ message: 'Medalha não encontrada' });
        }
