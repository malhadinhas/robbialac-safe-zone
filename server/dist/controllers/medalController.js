...
exports.deleteMedal = exports.updateMedal = exports.createMedal = exports.assignMedalToUser = exports.checkActionBasedMedals = exports.getUserUnacquiredMedals = exports.getUserMedals = exports.getMedals = void 0;
...
const getMedals = async (req, res) => {
    try {
        ...
        const medals = await Medal_1.default.find();
        logger_1.default.info('Medalhas recuperadas com sucesso', { count: medals.length });
        // Responde com o array de medalhas.
        res.json(medals);
    }
    catch (error) {
        // Captura e loga erros.
        logger_1.default.error('Erro ao recuperar medalhas', { error });
...
const getUserMedals = async (req, res) => {
    try {
        ...
        logger_1.default.info(`Buscando medalhas para o usuário: ${userId}`);
        // 1. Buscar as entradas na coleção 'user_medals' para este usuário.
        const userMedals = await UserMedal_1.default.find({ userId });
        // Se o usuário não tem nenhuma medalha, retorna um array vazio.
        if (userMedals.length === 0) {
            logger_1.default.info('Nenhuma medalha encontrada para o usuário', { userId });
        ...
        logger_1.default.debug(`Usuário ${userId} possui IDs de medalha:`, medalIds);
        // 3. Buscar os detalhes completos das medalhas na coleção 'medals' usando os IDs obtidos.
        const medals = await Medal_1.default.find({ id: { $in: medalIds } });
        logger_1.default.debug(`Detalhes das medalhas encontradas: ${medals.length} itens.`);
        ...
        logger_1.default.info('Medalhas do usuário recuperadas com sucesso', { userId, count: medalsWithEarnedDate.length });
        // Responde com o array das medalhas conquistadas, incluindo a data.
        res.json(medalsWithEarnedDate);
    }
    catch (error) {
        // Captura e loga erros.
        logger_1.default.error('Erro ao recuperar medalhas do usuário', { userId: req.params.userId, error });
        // Responde com erro 500.
        res.status(500).json({ message: 'Erro ao recuperar medalhas do usuário' });
    }
};
exports.getUserMedals = getUserMedals;
/**
 ...
 */
const getUserUnacquiredMedals = async (req, res) => {
    try {
        const { userId } = req.params; // ID do usuário da URL.
        logger_1.default.info(`Buscando medalhas não conquistadas pelo usuário: ${userId}`);
        ...
        logger_1.default.debug(`Usuário ${userId} possui IDs de medalha:`, acquiredMedalIds);
        // 2. Buscar TODAS as medalhas disponíveis no sistema (coleção 'medals').
        const allMedals = await Medal_1.default.find();
        logger_1.default.debug(`Total de medalhas no sistema: ${allMedals.length}`);
        ...
        logger_1.default.info('Medalhas não conquistadas recuperadas com sucesso', { userId, count: unacquiredMedals.length });
        // Responde com o array das medalhas não conquistadas.
        res.json(unacquiredMedals);
    }
    catch (error) {
        // Captura e loga erros.
        logger_1.default.error('Erro ao recuperar medalhas não conquistadas', { userId: req.params.userId, error });
        // Responde com erro 500.
        res.status(500).json({ message: 'Erro ao recuperar medalhas não conquistadas' });
    }
};
exports.getUserUnacquiredMedals = getUserUnacquiredMedals;
/**
 ...
 */
const checkActionBasedMedals = async (userId, activityCategory, activityDetails // Detalhes podem conter 'category' para vídeo/treino
) => {
    try {
        logger_1.default.info(`Verificando medalhas baseadas em ação para ${userId} após atividade ${activityCategory}`);
        // 1. Mapear a categoria da atividade para o 'triggerAction' correspondente da medalha.
        let targetTriggerAction = null;
        if (activityCategory === 'incident')
            targetTriggerAction = 'incidentReported';
        else if (activityCategory === 'video')
            targetTriggerAction = 'videoWatched';
        else if (activityCategory === 'training')
            targetTriggerAction = 'trainingCompleted';
        // Se a categoria da atividade não corresponde a nenhum gatilho de medalha, encerra.
        if (!targetTriggerAction) {
            logger_1.default.warn(`Categoria de atividade não mapeada para trigger de medalha: ${activityCategory}`, { userId });
            return [];
        }
        // 2. Buscar todas as medalhas que são acionadas por este tipo de ação (`triggerAction`).
        const medals = await Medal_1.default.find({ triggerAction: targetTriggerAction });
        // Se não há medalhas definidas para essa ação, encerra.
        if (medals.length === 0) {
            logger_1.default.info(`Nenhuma medalha encontrada no sistema para o trigger ${targetTriggerAction}`);
            return [];
        }
        logger_1.default.debug(`Encontradas ${medals.length} medalhas potenciais para ${targetTriggerAction}.`);
        ...
        logger_1.default.debug(`Usuário ${userId} já possui ${earnedMedalIds.length} medalhas. IDs:`, earnedMedalIds);
        ...
        if (medalsToCheck.length === 0) {
            logger_1.default.info(`Usuário ${userId} já possui todas as ${medals.length} medalhas disponíveis para ${targetTriggerAction} ou não há medalhas novas.`);
            return [];
        }
        logger_1.default.debug(`Verificando ${medalsToCheck.length} medalhas que o usuário ${userId} ainda não possui para ${targetTriggerAction}.`);
        ...
        let filterCategory = undefined; // Categoria específica para vídeo/treino
        ...
        if (targetTriggerAction === 'incidentReported') {
            ...
            if (!filterCategory) {
                ...
                logger_1.default.warn(`Detalhes da atividade de vídeo não contêm 'category' para ${userId}. Não é possível contar para medalhas específicas de categoria.`);
                ...
                logger_1.default.info(`Contagem de vídeos da categoria '${filterCategory}' para ${userId}: ${userActionCount}`);
            }
        }
            ...
            if (filterCategory) {
                ...
                logger_1.default.info(`Contagem de treinos da categoria '${filterCategory}' para ${userId}: ${userActionCount}`);
            }
            else {
                ...
                logger_1.default.info(`Contagem total de treinos para ${userId}: ${userActionCount}`);
            }
        }
        logger_1.default.info(`Contagem total da ação relevante (${targetTriggerAction}${filterCategory ? `/${filterCategory}` : ''}) para ${userId}: ${userActionCount}`);
        // 6. Verificar, para cada medalha pendente, se a contagem alcançou o necessário.
        const newlyEarnedMedals = []; // Array para armazenar as medalhas recém-ganhas.
        for (const medal of medalsToCheck) {
            ...
            if ((medal.triggerAction === 'videoWatched' || medal.triggerAction === 'trainingCompleted') && medal.triggerCategory) {
                ...
                if (filterCategory !== medal.triggerCategory) {
                    logger_1.default.debug(`Pula verificação da medalha ${medal.id}: categoria da atividade ('${filterCategory}') diferente da categoria requerida ('${medal.triggerCategory}').`);
            ...
            if (userActionCount >= medal.requiredCount) {
                ...
                logger_1.default.info(`Usuário ${userId} ALCANÇOU a medalha ${medal.name} (ID: ${medal.id}) com ${userActionCount}/${medal.requiredCount} ${medal.triggerAction} ${medal.triggerCategory ? `(cat: ${medal.triggerCategory})` : ''}`);
                ...
                await UserMedal_1.default.create({
                    userId, // ID do usuário
                    medalId: medal.id, // ID (string) da medalha
                    dateEarned: new Date(), // Data/hora atual
                });
                logger_1.default.info(`Medalha ${medal.id} ATRIBUÍDA a ${userId} na coleção 'user_medals'.`);
            }
            else {
                // Se a contagem não foi atingida.
                logger_1.default.debug(`Usuário ${userId} NÃO alcançou a medalha ${medal.id} (${medal.name}). Contagem: ${userActionCount}/${medal.requiredCount}.`);
            }
        }
        ...
        logger_1.default.error(`Erro ao verificar medalhas baseadas em ação para ${userId}`, { error, activityCategory, activityDetails });
 ...
 */
const assignMedalToUser = async (req, res) => {
    try {
        const { userId, medalId } = req.params; // IDs da URL.
        logger_1.default.info(`Tentativa de atribuir manualmente a medalha ${medalId} ao usuário ${userId}`);
        // Validação básica dos IDs.
        if (!userId || !medalId) {
            logger_1.default.warn('Atribuição manual de medalha: IDs de usuário ou medalha ausentes.', { userId, medalId });
            res.status(400).json({ message: 'IDs de usuário e medalha são obrigatórios' });
            return;
        }
        // 1. Verificar se a medalha existe no sistema.
        const medal = await Medal_1.default.findOne({ id: medalId });
        if (!medal) {
            logger_1.default.warn(`Atribuição manual: Medalha não encontrada com ID: ${medalId}`);
            res.status(404).json({ message: 'Medalha não encontrada' });
            return;
        }
        // 2. Verificar se o usuário já possui esta medalha para evitar duplicatas.
        const existingMedal = await UserMedal_1.default.findOne({ userId, medalId });
        if (existingMedal) {
            logger_1.default.info(`Usuário ${userId} já possui a medalha ${medalId} (atribuída em ${existingMedal.dateEarned}). Atribuição manual ignorada.`);
            res.status(200).json({
        ...
        await UserMedal_1.default.create(userMedal);
        logger_1.default.info(`Medalha ${medalId} atribuída manualmente com sucesso ao usuário ${userId}`);
        ...
        logger_1.default.info(`Atividade registrada para atribuição manual da medalha ${medalId} ao usuário ${userId}`);
        ...
        logger_1.default.error('Erro ao atribuir medalha manualmente ao usuário', { error, params: req.params });
        // Responde com erro 500.
 ...
 */
const createMedal = async (req, res) => {
    try {
        ...
        logger_1.default.info('Requisição para criar nova medalha recebida.', { data: medalData });
        ...
        if (!medalData.id || !medalData.name || !medalData.description || !medalData.imageSrc || !medalData.triggerAction || !medalData.requiredCount) {
            logger_1.default.warn('Tentativa de criar medalha com dados incompletos', medalData);
        ...
        if (medalData.requiredCount <= 0) {
            logger_1.default.warn(`Contagem inválida na criação da medalha ${medalData.id}: ${medalData.requiredCount}`);
        ...
        if ((medalData.triggerAction === 'videoWatched' || medalData.triggerAction === 'trainingCompleted') && !medalData.triggerCategory) {
            logger_1.default.warn(`Categoria não fornecida para medalha ${medalData.id} com trigger ${medalData.triggerAction}`);
        ...
        logger_1.default.info(`ID normalizado para a nova medalha: ${medalData.id}`);
        // Obtém a coleção 'medals'.
        const medal = await Medal_1.default.create(medalData);
        logger_1.default.info(`Nova medalha criada com sucesso: ${medalData.name} (ID: ${medalData.id}), _id: ${medal._id}`);
        // Responde com 201 Created e o documento da medalha criada.
        res.status(201).json(medal);
    }
    catch (error) {
        // Captura e loga erros gerais.
        logger_1.default.error('Erro ao criar medalha', { error, body: req.body });
        // Responde com erro 500.
...
const updateMedal = async (req, res) => {
    try {
        ...
        logger_1.default.info(`Requisição para atualizar medalha recebida: ${medalId}`, { updateData });
        if (Object.keys(updateData).length === 0) {
            logger_1.default.warn(`Nenhum dado fornecido para atualização da medalha ${medalId}`);
        ...
        if (updateData.requiredCount !== undefined && updateData.requiredCount <= 0) {
            logger_1.default.warn(`Contagem inválida na atualização da medalha ${medalId}: ${updateData.requiredCount}`);
        ...
        if ((updateData.triggerAction === 'videoWatched' || updateData.triggerAction === 'trainingCompleted') && updateData.triggerCategory === undefined) {
            logger_1.default.warn(`Atualização de medalha ${medalId} pode resultar em estado inválido (ação vídeo/treino sem categoria) - validação incompleta.`);
        }
        delete updateData.id;
        const updatedMedal = await Medal_1.default.findOneAndUpdate({ id: medalId }, { $set: { ...updateData, updated_at: new Date() } }, { new: true });
        if (!updatedMedal) {
            logger_1.default.warn(`Tentativa de atualizar medalha não encontrada: ${medalId}`);
            res.status(404).json({ message: 'Medalha não encontrada' });
            return;
        }
        logger_1.default.info(`Medalha atualizada com sucesso: ${medalId}`);
        res.status(200).json(updatedMedal);
    }
    catch (error) {
        logger_1.default.error(`Erro ao atualizar medalha ${req.params.medalId}`, {
            error: error.message,
            stack: error.stack,
...
exports.updateMedal = updateMedal;
/**
...
const deleteMedal = async (req, res) => {
    try {
        ...
        logger_1.default.info(`Requisição para deletar medalha: ${medalId}`);
        // Tenta deletar a medalha que corresponde ao ID (slug).
        const result = await Medal_1.default.deleteOne({ id: medalId });
        // Verifica se alguma medalha foi deletada.
        if (result.deletedCount === 0) {
            logger_1.default.warn(`Tentativa de deletar medalha não encontrada: ${medalId}`);
        ...
        */
        logger_1.default.info(`Medalha deletada com sucesso: ${medalId}`);
        ...
        // Captura e loga erros gerais.
        logger_1.default.error(`Erro ao deletar medalha ${req.params.medalId}`, { error });
        // Responde com erro 500.
        res.status(500).json({ message: 'Erro interno ao deletar medalha' });
    }
};
exports.deleteMedal = deleteMedal;
