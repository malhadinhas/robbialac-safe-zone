...
exports.getLeaderboard = exports.getUserRanking = exports.getUserPointsBreakdown = void 0;
 ...
 */
const getUserPointsBreakdown = async (req, res) => {
    try {
        const { userId } = req.params;
        if (!userId) {
            logger_1.default.warn('Solicitação de pontos sem ID de usuário');
            res.status(400).json({ message: 'ID de usuário é obrigatório' });
            return;
        }
        logger_1.default.info(`Buscando distribuição de pontos para o usuário ${userId}`);
        // Consulta as atividades do usuário agrupadas por categoria
        const pointsByCategory = await UserActivity_1.default.aggregate([
            { $match: { userId } },
            { $group: {
                    _id: "$category",
        ...
        const result = pointsByCategory.map(item => ({
        ...
        logger_1.default.info(`Distribuição de pontos recuperada com sucesso`, { userId, categories: result.length });
        res.json(result);
    }
    catch (error) {
        logger_1.default.error('Erro ao buscar distribuição de pontos do usuário', {
...
exports.getUserPointsBreakdown = getUserPointsBreakdown;
/**
...
const getUserRanking = async (req, res) => {
    try {
        const { userId } = req.params;
        if (!userId) {
            logger_1.default.warn('Solicitação de ranking sem ID de usuário');
            res.status(400).json({ message: 'ID de usuário é obrigatório' });
            return;
        }
        logger_1.default.info(`Buscando ranking para o usuário ${userId}`);
        // Obtém a coleção de usuários
        const usersCollection = await User_1.default.find();
        ...
        logger_1.default.info(`Ranking recuperado com sucesso`, {
            userId,
        ...
        logger_1.default.error('Erro ao buscar ranking do usuário', {
            userId: req.params.userId,
            error: error instanceof Error ? error.message : 'Erro desconhecido'
        });
        res.status(500).json({
...
const getLeaderboard = async (req, res) => {
    try {
        ...
        const users = await User_1.default.find({}, { name: 1, points: 1 }).lean();
        // Buscar todas as medalhas
        const allMedals = await Medal_1.default.find({}, { id: 1, name: 1, imageSrc: 1, requiredPoints: 1 }).lean();
        const medalsMap = new Map(allMedals.map(m => [m.id, m]));
        // Obter medalhas e top 3 para cada usuário
        const leaderboardData = await Promise.all(users.map(async (user) => {
            const userMedalsDocs = await UserMedal_1.default.find({ userId: user._id.toString() }).lean();
        ...
        logger_1.default.error('Erro ao obter leaderboard', { error });
