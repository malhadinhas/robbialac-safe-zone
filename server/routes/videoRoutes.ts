...
import { generateSignedUrl, generateUploadUrl } from '../services/cloudflareR2Service';
...
router.post('/category/:category/next', async (req, res) => {
  try {
    ...
    const objectIds = viewedVideoIds.map((id: string) => new ObjectId(id));
...
router.get('/:id/stream', async (req, res) => {
  try {
    ...
    const url = await generateSignedUrl(video._id.toString());
    res.json({ url });
  } catch (error) {
...
router.post('/:id/upload', async (req, res) => {
  try {
    ...
    const url = await generateUploadUrl(req.params.id, contentType);
    res.json({ url });
  } catch (error) {
